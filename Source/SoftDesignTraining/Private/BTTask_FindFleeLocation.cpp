...
EBTNodeResult::Type UBTTask_FindFleeLocation::ExecuteTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory)
{
    ...
    if (bestFleeLocation)
    {
        ...
        {
            if (ASDTAIController* AIController = Cast<ASDTAIController>(aiController))
            {
                OwnerComp.GetBlackboardComponent()->SetValue<UBlackboardKeyType_Vector>(AIController->GetTargetPosBBKeyID(), bestFleeLocation->GetActorLocation());
            }
        }

        //if (ASDTAIController* AIController = Cast<ASDTAIController>(aiController))
        //{
        //    AIController->m_ShouldGoToFleeLocation = false;
        //}
